...
```
Finally we call `select` with arguments we prepared earlier. `select()` accepts 3 `fd_set`s (we ignore the third for this example) and a timeout and returns something non-zero in case one (or more) of the file descriptors in the sets are ready. We should then go and find which one it was!
...
In this section, I'd like to recap how a future (let's take [`Read`](https://rust-lang-nursery.github.io/futures-doc/0.3.0-alpha.2/futures/io/struct.Read.html)
 ...
 - First it's created using the [`read()`](https://rust-lang-nursery.github.io/futures-doc/0.3.0-alpha.2/futures/io/trait.AsyncReadExt.html#method.read)
    method on `AsyncTcpStream`. This method is implemented for all types that implement the `AsyncRead` trait.
 - Then it's spawned on the executor, using either the `run()` or the `spawn()` method.
 - The executor calls the `poll()` method on the future. The `poll` [implementation](https://rust-lang-nursery.github.io/futures-doc/0.3.0-alpha.2/src/futures_util/io/read.rs.html#28)
    for `Read` calls `poll_read()` on the `AsyncTcpStream`, which registers its interest in `readable` events.
 - When an event occurs, the future is polled again. This cycle is repeated until the future is ready.

## Thank you
Thanks to the whole futures team, and specially to [@aturon](https://github.com/aturon) for the encouragement and to [@MajorBreakfast](https://github.com/MajorBreakfast) for his edits.
